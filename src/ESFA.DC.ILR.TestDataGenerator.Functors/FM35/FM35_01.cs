...
//iii.    The zprog + second then complete. The achievement payment for the 1st component should then appear when the zprog is achieved
                ...
                new LearnerTypeMutator() { LearnerType = LearnerTypeRequired.Apprenticeships, DoMutateLearner = MutateOlderApprenticeshipMultiYearComplexRestart, DoMutateOptions = MutateGenerationOptionsOlderApprenticeshipLD10, DoMutateProgression = MutateOlderApprenticeshipComplexRestartsDestAndProg },
            };
        }
        ...
        private void MutateOlderApprenticeshipMultiYearComplexRestart(MessageLearner learner, bool valid)
        {
            Mutate19(learner, valid);
            Helpers.MutateApprenticeshipToOlderWithFundingFlag(learner, LearnDelFAMCode.FFI_Co);
            Helpers.MutateDOB(learner, valid, Helpers.AgeRequired.Exact19, Helpers.BasedOn.LearnDelStart, Helpers.MakeOlderOrYoungerWhenInvalid.NoChange);
        ...
        {
            _options = options;
            options.LD.OverrideLearnStartDate = DateTime.Parse("2016-DEC-21");
            options.LD.IncludeHHS = true;
